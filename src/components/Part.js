...
// console.log(props.location.state.part.partInfo.vendor_name);
const [vendor_name, setVendor] = React.useState(props.location.state.part.partInfo.vendor_name.vendor);
const [vendor_number, setVendorNumber] = React.useState(props.location.state.part.partInfo.vendor_number.vendor_number);
const [part_number, setPartNumber] = React.useState(props.location.state.part.partInfo.part_number.part_number);
const [container_number, setContainer] = React.useState(props.location.state.part.partInfo.container_number.container_number);

    // console.log(UserProfile.getName());
    const {rootState, partSearch} = useContext(MyContext);
    // useEffect(() => {
    //     partSearch();
    //   }, []);
    const {partData} = rootState;     
localStorage.setItem('partData', partData);
    ...
    const handleSelectAllClick = (event) => {
      if (event.target.checked) {
        const newSelecteds = partData.map((n) => n.id);
    ...
    function handleChange(event) {
      ...
      let _vals = event.target.value
        ? partData.filter(r => r.active === event.target.value)
        : partData;
        // console.log(_vals);
      setTableData(_vals);
    }
    ...
    return (
      ...
      <div className={classes.root}>
              ...
              <SearchAppBar data={partData} handleChange={handleChange} selectedItem={selectedItem}/>
